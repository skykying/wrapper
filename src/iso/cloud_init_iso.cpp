...
void mp::CloudInitIso::write_to(const Path& path, const std::string& meta_data, const std::string& user_data)
{
    QFile f{path};
    if (!f.open(QIODevice::WriteOnly))
        throw std::runtime_error{"failed to open file for writing during cloud-init generation"};

    // First 32KB are reserved
    f.seek(32768);

    PrimaryVolumeDescriptor prim_desc;
    JolietVolumeDescriptor joliet_desc;
    VolumeDescriptorSetTerminator terminator;

    const uint32_t num_descriptors = 3u;
    const uint32_t num_path_tables = 2u;
    const uint32_t num_blocks_for_dir_records = 2u;
    const auto meta_data_blocks = num_blocks(meta_data.size());
    const auto user_data_blocks = num_blocks(user_data.size());

    const auto volume_size =
        16u + num_descriptors + num_path_tables + num_blocks_for_dir_records + meta_data_blocks + user_data_blocks;
    prim_desc.set_volume_size(volume_size);
    joliet_desc.set_volume_size(volume_size);

    uint32_t block_index = 16u + num_descriptors;
    RootPathTable root_path{block_index + num_path_tables};
    prim_desc.set_path_table_info(root_path.data.size(), block_index);
    ++block_index;

    RootPathTable joliet_root_path{block_index + num_path_tables};
    joliet_desc.set_path_table_info(joliet_root_path.data.size(), block_index);
    ++block_index;

    RootDirRecord root_record{RootDirRecord::Type::root, block_index};
    RootDirRecord root_parent_record{RootDirRecord::Type::root_parent, block_index};
    prim_desc.set_root_dir_record(root_record);

    ++block_index;
    RootDirRecord joliet_root_record{RootDirRecord::Type::root, block_index};
    RootDirRecord joliet_root_parent_record{RootDirRecord::Type::root_parent, block_index};
    joliet_desc.set_root_dir_record(joliet_root_record);

    ++block_index;
    const uint32_t meta_data_block_start = block_index;
    const uint32_t user_data_block_start = meta_data_block_start + meta_data_blocks;

    uint32_t meta_data_size = meta_data.size();
    uint32_t user_data_size = user_data.size();
    ISOFileRecord meta_data_record{"meta-data", meta_data_block_start, meta_data_size};
    ISOFileRecord user_data_record{"user-data", user_data_block_start, user_data_size};
    JolietFileRecord joliet_meta_data_record{"meta-data", meta_data_block_start, meta_data_size};
    JolietFileRecord joliet_user_data_record{"user-data", user_data_block_start, user_data_size};
    ...
    write(meta_data_record, f);
    write(user_data_record, f);
    seek_to_next_block(f);

    write(joliet_root_record, f);
    write(joliet_root_parent_record, f);
    write(joliet_meta_data_record, f);
    write(joliet_user_data_record, f);
    seek_to_next_block(f);

    f.write(meta_data.data(), meta_data.size());
    seek_to_next_block(f);

    f.write(user_data.data(), user_data.size());
    pad_to_end(f);
}
